...
function renderPanel(
  props: { busy?: boolean; onBusyChange?: (b: boolean) => void } = {},
) {
...
describe("SubscriptionsPanel", () => {
  ...
  it("shows subscriber count and API key status", async () => {
    ...
    await waitFor(() =>
      expect(screen.getAllByText(/configured/i)).toHaveLength(1),
    );
    expect(
      screen.queryByLabelText(/webhook signing secret/i),
  ...
  it("shows loading state initially, then content", async () => {
    ...
    vi.mocked(apiMod.fetchSubscriptionSettings).mockReturnValue(
      new Promise<typeof FULL_SETTINGS>((r) => {
        resolve = r;
      }),
    );
    renderPanel();
  ...
  it("shows the required user-provided settings when enable is not allowed", async () => {
    ...
    expect(
      screen.getByText(/requires api key \+ from email/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows 400 detail error from updateSubscriptionSettings", async () => {
    vi.mocked(apiMod.updateSubscriptionSettings).mockRejectedValue(
      new MockHTTPError(
        400,
        JSON.stringify({ detail: "Enable precondition not met." }),
      ),
    );
    const user = userEvent.setup();
    renderPanel();
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/enable precondition not met/i),
      ).toBeInTheDocument(),
    );
  });
  ...
  it("loads published posts from every posts page", async () => {
    ...
    expect(
      await screen.findByRole("option", { name: "Older Published Post" }),
    ).toBeInTheDocument();
  ...
  it("shows completed status only after the ledger reports sent", async () => {
    ...
    expect(
      screen.queryByText(/broadcast queued|broadcast started/i),
    ).not.toBeInTheDocument();
  });
  ...
  it("calls sendTestEmail from test-email control", async () => {
    const user = userEvent.setup();
    vi.mocked(apiMod.sendTestEmail).mockResolvedValue({
      message: "Test email sent.",
    });
    renderPanel();
    await screen.findByText(/42/);
    const emailInput = screen.getByRole("textbox", {
      name: /test email address/i,
    });
  ...
  it("shows an error banner when the mount load fails", async () => {
    ...
    vi.mocked(apiMod.fetchSubscriptionSettings).mockRejectedValue(
      new Error("network"),
    );
    renderPanel();
    await waitFor(() =>
      expect(
        screen.getByText(/failed to load subscription settings/i),
      ).toBeInTheDocument(),
    );
  });
