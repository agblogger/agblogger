...
All local state lives in two durable Alembic-managed tables: a singleton settings row (enabled flag, encrypted Resend API key and webhook signing secret, segment id, sender identity, and optional GDPR compliance fields) and a broadcast ledger with one row per send attempt — request correlation id, post reference, trigger, status, and error, but never recipient data.
...
Because email clients ignore external and most embedded CSS, the post body is restyled with inline styles that mirror the on-site `.prose` rendering (body type, headings, links, syntax-highlighted code blocks, inline code, blockquotes, note callouts, tables, and smaller footnotes); the web font stack falls back to system equivalents, Pandoc highlight token classes are mapped to inline colors (the Catppuccin palette), and pseudo-element labels (e.g. the note "Note" tag) become real markup. The style map is a deliberate, email-adapted subset of `frontend/src/index.css` — keep the two in sync when post typography changes.

Because email clients run no JavaScript, KaTeX cannot render client-side as it does on the web (see [editor.md](editor.md)); math spans are instead rewritten into images served by an external LaTeX rendering service (`latex.codecogs.com`), with the raw TeX as alt text so blocked images still read. Each distinct formula is rendered at a high DPI and displayed at a measured, text-matched size: the build fetches each image once (concurrently) to read its pixel dimensions and sets explicit `width`/`height`, so formulas stay sharp on high-density screens while matching surrounding text. If a measurement fetch fails, the formula falls back to a correctly-sized (display-DPI) image with no explicit dimensions. This involves a tradeoff: the image request leaks the reader's IP to the third-party service and acts as an open-tracking signal; availability depends on the external service, and the build performs a bounded set of measurement requests per broadcast. No alternative local rendering path exists.

## Enable Precondition

Enabling subscriptions requires only a Resend API key and sender address from the operator. AgBlogger registers a signed Resend webhook for `contact.updated` events and permanently deletes the contact when the event explicitly reports `unsubscribed: true`. The provider-generated signing secret is encrypted with the application secret. Resend generates the signing secret because Resend signs webhook requests; it cannot be independently derived by AgBlogger. Webhook setup is best-effort because Resend requires a public HTTPS endpoint: localhost and temporary provider failures do not block subscribing, confirmation, test emails, or broadcasts. While the webhook is missing, the admin panel warns that unsubscribed contacts will not be automatically deleted from Resend. GDPR compliance fields (controller name, controller contact, privacy policy URL, postal address) are optional — the subscribe page renders each part of the GDPR notice conditionally based on what is configured, and the public site config exposes the compliance object only while subscriptions are enabled.

The first enable lazily auto-creates the Resend segment and attempts to create the unsubscribe webhook. Changing the Resend API key invalidates the account-bound segment and webhook secret so both are provisioned against the replacement account. If the current URL is not HTTPS, webhook registration is skipped without calling Resend. If registration fails or was skipped, every later settings update that leaves subscriptions enabled retries when the request uses HTTPS. Every enabled settings update also revalidates the required API key and sender address, so those fields cannot be cleared while the feature is active. Missing-field and required Resend-resource errors surface as client errors so the admin UI can show a useful message.
