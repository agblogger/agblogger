...
Posts and pages display backend HTML through one shared `RenderedContent` component (`frontend/src/components/RenderedContent.tsx`), so both get identical presentation: KaTeX hydration plus code-block language headers and copy buttons. The body is rendered verbatim — no headings are stripped. A page's title comes solely from site config (`index.toml`) and is rendered separately by the page view, so the page body owns its own in-body headings.
...
- `frontend/src/components/admin/SubscriptionsPanel.tsx` is the admin Subscriptions tab.
