...
- `frontend/src/pages/SubscribePage.tsx` is the public email opt-in form with GDPR layered notice, served at `/subscribe`.
- `frontend/src/components/admin/SubscriptionsPanel.tsx` is the admin Subscriptions tab for settings, enable toggle, subscriber count, test email, manual broadcast, and broadcast history. The header link to `/subscribe` and the Subscriptions tab are both gated on the `subscriptions_enabled` flag exposed in the `GET /api/pages` site config response.
