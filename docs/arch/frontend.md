...
- `frontend/src/pages/SubscribePage.tsx` is the public email opt-in form with a GDPR layered notice populated from enabled subscription compliance settings, served at `/subscribe`.
- `frontend/src/components/admin/SubscriptionsPanel.tsx` is the admin Subscriptions tab for settings, required unsubscribe-webhook configuration, enable toggle, subscriber count, test email, manual broadcast, and broadcast history. It paginates through the full post list for manual broadcasts, polls history after a manual trigger until the attempt is visible, and refreshes shared site config after toggles. The header link to `/subscribe` and the Subscriptions tab are both gated on the `subscriptions_enabled` flag exposed in the `GET /api/pages` site config response.
