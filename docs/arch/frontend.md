...
- `frontend/src/components/admin/SubscriptionsPanel.tsx` is the admin Subscriptions tab for settings, enable toggle, subscriber count, test email, manual broadcast, and broadcast history. It paginates through the full post list for manual broadcasts and refreshes shared site config after toggles. The header link to `/subscribe` and the Subscriptions tab are both gated on the `subscriptions_enabled` flag exposed in the `GET /api/pages` site config response.
