...
Admin dashboard statistics — total views, per-path hits, referrers, browser and OS breakdowns — are proxied from GoatCounter's stats API through admin-only backend endpoints. The frontend computes the selected local-day window in the browser and sends explicit UTC start/end instants. Stats are only served while analytics is enabled. The frontend reads settings first and short-circuits stats fetches when analytics is disabled, returning zeroed-out data so the normal off state shows empty dashboards rather than loading spinners or GoatCounter outage errors.
