...
The sync client is a separate CLI companion that uses the same authenticated API boundary as the browser and other clients. It authenticates via interactive username/password session login, stores refresh credentials per server, carries the CSRF token required for unsafe cookie-authenticated requests, and persists each rotated refresh token when access tokens expire during long runs. Stored credentials fall back to interactive login only when rejected by the server. The client does not bypass the application by reading or writing server files directly.
