...
- **Credential hashing**: Refresh tokens, PATs, and invite codes stored as one-way hashes (SHA-256 for tokens, bcrypt for passwords).
...
- **Session revocation on password change** (`backend/api/admin.py:246`): All refresh tokens and PATs revoked when password is changed.
- **DB-authoritative roles** (`backend/api/deps.py:130`): `get_current_user` fetches the user from the DB, so `is_admin` always reflects current state regardless of JWT claims.
...
- Social accounts are scoped to `user.id` -- users can only manage their own accounts.

## Draft Visibility: Properly Enforced

Drafts are correctly hidden from public and non-author users across all read paths:

| Path | Mechanism | Location |
|------|-----------|----------|
| `GET /api/posts` (list) | `draft_owner_username` filter: shows published + author's own drafts | `post_service.py:96-107` |
...
| `POST /api/auth/register` | None / invite code | Controlled by config |
...
| `PATCH /api/auth/me` | `require_auth` | |
| `POST /api/auth/invites` | `require_admin` | |
| `POST/GET/DELETE /api/auth/pats` | `require_auth` | Scoped to user |
| `GET /api/posts` | `get_current_user` (optional) | Draft filtering applied |
| `GET /api/posts/search` | None | Drafts excluded in FTS query |
| `GET /api/posts/{path}` | `get_current_user` (optional) | Draft filtering applied |
...
| `GET /api/labels` | `get_current_user` (optional) | Post counts filtered |
| `GET /api/labels/{id}` | `get_current_user` (optional) | Post count filtered |
...
| `GET /api/pages/{id}` | `get_current_user` (optional) | Pages are always public |
...
| `POST/GET/DELETE /api/crosspost/accounts` | `require_auth` | Scoped to user |
| `POST /api/crosspost/post` | `require_auth` | Uses user's own accounts |
| `GET /api/crosspost/history/{path}` | `require_auth` | Scoped to user |
| OAuth authorize endpoints | `require_auth` | |
| OAuth callback endpoints | State token (anti-CSRF) | Standard OAuth pattern |
| `GET /api/content/{path}` | `get_current_user` (optional) | Draft access checked |
...
This endpoint does not inject `get_current_user` and doesn't pass `draft_owner_username` to `get_posts_by_label`. As a result, even the post author can't see their own draft posts when filtering by label. This is a **feature gap**, not a security issue -- drafts are hidden (conservative default), they're just also hidden from the author on this specific endpoint.
...
### 3. Cross-posting uses `require_auth` instead of `require_admin`

**Location:** `backend/api/crosspost.py:208`

Non-admin authenticated users can cross-post published posts using their own connected social accounts. This appears intentional per the architecture docs ("authenticated users can perform account-scoped actions"), but worth confirming if this is the desired behavior in the deployment model.

### 4. Health endpoint exposes version
...
- `backend/api/deps.py` -- Auth dependencies (`get_current_user`, `require_auth`, `require_admin`)
- `backend/api/auth.py` -- Auth endpoints (login, register, refresh, logout, CSRF, profile, PATs, invites)
